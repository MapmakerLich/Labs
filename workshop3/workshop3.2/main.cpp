...
{
    if ((ball.position.x + BALL_SIZE >= WINDOW_WIDTH) && (ball.speed.x > 0))
    {
    ...
    if ((ball.position.y + BALL_SIZE >= WINDOW_HEIGHT) && (ball.speed.y > 0))
    {
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        const float dtPhysics = deltaTime / MAX_INTERVAL_NUMBER;
